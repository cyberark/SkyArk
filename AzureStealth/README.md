...
**Discovers the most privileged Users in the target Azure environments - including the stealthy Azure Shadow Admins.**  
...
Potential attackers are hunting for those kinds of users. The defensive teams must make sure these privileged users are well secured - have strong, rotated and safety stored credentials, have MFA enabled, being monitored carefully, etc.  
Remember that we cannot protect the things we don’t aware of, and AzureStealth will help in the complex mission of discovereing the most privileged users - the straight-forward admins and the unique stealthy shadow admins that could also easily escalate privileges and become full admins.
...
## RUN AZURESTEALTH DIRECTLY FROM AZURE'S BUILD-IN POWERSHELL CLOUDSHELL:  
